# RAG_QA_Test.py — 检索 HitRate / MRR 评测（HYBRID=True 为混合检索, False 为纯向量）
...
async def main():
    ...
    mode = "混合检索(向量+BM25+RRF)" if HYBRID else "纯向量"
    print(f"测试集 {len(testset)} 条, 模式={mode}, 开始检索 ...")

    # 每题只查一次: 取 Top-MAX_K、不设阈值, 带着分数返回
    cache = []   # cache[i] = [(file_name, score), ...] 按相似度降序
    for i, item in enumerate(testset):
        results = await retrieve(item["question"], COLLECTION,
                                 k=MAX_K, score_threshold=None, hybrid=HYBRID)
